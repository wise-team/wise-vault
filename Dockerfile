...
RUN apt-get update && apt-get install -y openssl && mkdir /certgen
...
RUN cat /certgen/localhost.conf

RUN openssl req -x509 -out /certgen/localhost.crt -keyout /certgen/localhost.key \
  -newkey rsa:2048 -nodes -sha256 \
  -days 365 \
  -subj '/CN=localhost' -extensions EXT -config /certgen/localhost.conf
